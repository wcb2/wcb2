/* $Id: commands.c 5053 2010-11-19 19:08:08Z wiechu $ */
		...
		if (bookmark_sync) {
			...
			switch (bookmark_sync) {
				...
				{       
					...
					error = add_bookmark(j, splitted);
					if (error) bookmark_sync = -1;
				}
				break;
				...
				case (2):	 
				{
					...
					if (!bookmarks) {
						...
						bookmark_sync = -2; break;
					}

					if (xstrcmp(splitted[0], "name") && xstrcmp(splitted[0], "url") && xstrcmp(splitted[0], "conf")) {
						bookmark_sync =  -1; break;
					}

					book = find_bookmark(splitted[0], splitted[1], bookmarks);
					if (book == NULL) {
						printq("generic_error", "No such bookmark");
						bookmark_sync =  -2; break;
					}

					error = modify_bookmark(book, splitted + 2);
					if (error) bookmark_sync = -1;
				}
				break;
...
{	
	...
	if (!xstrcmp(params[0], "moderator"))
		role = "moderator";
	else if (!xstrcmp(params[0], "participant"))
		role = "participant";
	else if (!xstrcmp(params[0], "visitor"))
		role = "visitor";
...
#define JABBER_FLAGS_MSG		JABBER_ONLY | COMMAND_ENABLEREQPARAMS | COMMAND_PARAMASTARGET
	...
	command_add(&jabber_plugin, "xmpp:affiliation", "!p !nN ?", jabber_muc_command_affiliation, JABBER_FLAGS_TARGET, 
	...
	command_add(&jabber_plugin, "xmpp:bookmark", "!p ?p", jabber_command_private, JABBER_FLAGS_REQ,
			"-a --add -c --clear -d --display -j --join -m --modify -r --remove --conf --url --name");
	...
	command_add(&jabber_plugin, "xmpp:muc_lists", "!p !", jabber_muc_command_muc_lists, JABBER_FLAGS_TARGET, 
			"affiliation role");
	...
	command_add(&jabber_plugin, "xmpp:role", "!p !nN ?", jabber_muc_command_role, JABBER_FLAGS_TARGET, 
